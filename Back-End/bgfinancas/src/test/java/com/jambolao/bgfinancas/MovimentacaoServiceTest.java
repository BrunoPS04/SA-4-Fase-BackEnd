...
public class MovimentacaoServiceTest {
    ...
    void setUp() {
        ...
        movimentacao1 = new Movimentacao((long) 8, "Deposito 1", (float) 17.99, "Entrada", categoria1, LocalDate.now(),
                null);
        movimentacao2 = new Movimentacao((long) 9, "Retirada 1", (float) 17.99, "Saida", categoria2, LocalDate.now(),
                null);
    }

    @Test
    void shouldCreateMovimentacao() {
        MovimentacaoRequestDTO movimentacaoDTO = new MovimentacaoRequestDTO(
                movimentacao1.getDescricao(),
                movimentacao1.getValor(),
                movimentacao1.getTipo(),
                movimentacao1.getCategoria().getNomeCategoria(),
                movimentacao1.getData(),
                null);

        when(movimentacaoRepository.save(any(Movimentacao.class))).thenReturn(movimentacao1);

        Movimentacao createdMovimentacao = movimentacaoService.createMovimentacao(movimentacaoDTO);
